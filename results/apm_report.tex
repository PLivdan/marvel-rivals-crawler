...
\fancyfoot[C]{\thepage}\fancyfoot[R]{\scriptsize build \texttt{f3e37420}}
...
{\small Season 9.5, PC ranked. Prepared 8 September 2026. Model fit \texttt{31cfdff}, report build \texttt{f3e37420}.}
\end{center}
...
defeat most of the purpose of having 477,483 matches in the first place.
...
with three Damage heroes wins 42.2\% of its matches, and two Tanks with three Supports wins
40.4\%. If composition were ignored, the regression would happily blame individual heroes
...
between the two teams, and the rare remainder is pooled into a single ``other'' category. The point is simple. The composition block gets to
...
+7.94 and then +14.57 across the three rules. The interesting part is that the rankings do
not collapse. Their rank correlations with the starting-hero ranking are still 0.937 for
the most-played rule and 0.890 for play-time weighting. What changes is the magnitude. The
...
within-role estimates (filled circles), all fitted on the same 477,483 matches, and
the estimate for Magik (open circles), the hero whose estimate moves most between the
...
0.010 percentage points. Fixing the identification problem does not manufacture the
...
These numbers are marginal effects evaluated at a balanced match. The Thing's 5.46 does not
mean that every team picking The Thing raises its observed win rate by 5.46 points. It means
that, at the evaluation point, replacing an average Tank with The Thing is associated with about
a 5.46 percentage-point increase in predicted win probability after conditioning on the
controls in the model. 10 of the 55 heroes cannot be statistically distinguished from
...
seen. I therefore fit the model on the 437,453 matches played before the final
...
correlation of 0.957 and a mean absolute gap of 0.50 percentage points. More
revealingly, if the hero terms are removed while map, rank score, composition, and team-ups
are retained, the correlation falls to 0.139. The remaining controls know something
...
\node[font=\tiny, anchor=north west, inner sep=0.6pt, fill=white, fill opacity=0.85, text opacity=1] at (axis cs:0.4430,0.4385) {Deadpool (Strategist)};
\draw[very thin] (axis cs:0.4993,0.4863) -- (axis cs:0.4693,0.5163);
\node[font=\tiny, anchor=south east, inner sep=0.6pt, fill=white, fill opacity=0.85, text opacity=1] at (axis cs:0.4693,0.5163) {Hela};
\draw[very thin] (axis cs:0.5027,0.5273) -- (axis cs:0.5327,0.4973);
\node[font=\tiny, anchor=north west, inner sep=0.6pt, fill=white, fill opacity=0.85, text opacity=1] at (axis cs:0.5327,0.4973) {Human Torch};
...
\node[font=\tiny, anchor=north west, inner sep=0.6pt, fill=white, fill opacity=0.85, text opacity=1] at (axis cs:0.3319,0.2850) {1-4-1};
\node[font=\tiny, anchor=south west, inner sep=0.6pt, fill=white, fill opacity=0.85, text opacity=1] at (axis cs:0.3498,0.3518) {3-2-1};
...
\emph{Notes.} The model is fitted on the 437,453 matches before the final 7
days and evaluated on the 40,030 matches of that week. Each holdout match contributes
...
team-ups takes it to 0.6674 (Appendix Table~\ref{tab:ladder}). The hero terms are
...
52.52\% across the 16 maps. None of these checks proves that the coefficients are
...
Adam Warlock with Ultron 6.82. These values are not the total strength of the pairs. If two
individually excellent heroes also have a positive team-up coefficient, their combined
contribution is both hero terms plus the premium. Reading 8.66 as the gain from fielding
Squirrel Girl and Iron Man together counts the wrong object. The coefficient measures what the pair
...
+3.47 percentage points, much more caution is warranted.
...
656 hours of computation at the current sample size. This does not change the point
estimates. It does mean the uncertainty around them should be read more conservatively than
the HC1 intervals suggest.

Finally, the tails of the data are imperfect. 18 composition shapes receive their own indicators, while 9 extremely rare shapes, together only 0.024\% of team instances, share the ``other'' category and one coefficient. The crawl is also not a
random sample of the ranked population. Profile privacy rises sharply at high rank scores and
reaches roughly 63\% above 5{,}000, and because the crawl expands only through public
...
an average hero filling the same role? On 477,483 matches, the answer is not remotely
