...
from the final 7 days and evaluated on the 40,030 matches of that week,
...
the points lie along the $45^\circ$~line, with a correlation of 0.957 and a mean
absolute gap of 0.50 percentage points, whereas a model with every control except
the hero terms spreads its predictions across heroes with a standard deviation of only
0.44 points against 2.23 for the full model. The same holds by team
...
1.067 (standard error 0.025), slightly above the ideal of one, so if anything the model is mildly under-confident out of sample.
...
55-hero family: $^{*}$~$q<0.10$, $^{**}$~$q<0.05$, $^{***}$~$q<0.01$; 10 of
